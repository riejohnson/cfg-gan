...
int max_threads = -1; 
int max_blocks = -1; 
/* bool __doDebug = false; moved to AzUtil.cpp 2/10/2018 */
/* bool __doTry = false;   moved to AzUtil.cpp 2/10/2018 */
/*----------------*/
...
void AzPmat::set(const AzPmat *m, const AzPintArr2v &pia2v_cols, int idx, AzFloat coeff) {
  ...
  AzX::throw_if_null(m, eyec, "m");
  AzX::throw_if((cnum <= 0), eyec, "#columns must be positive"); 
...
/*------------------------------------------------*/
/*!!!!! For speed, trust pia_scol.                */
void AzPmat::copy_scol(const AzPmat &m, const AzPintArr &pia_scol) {
  AzX::throw_if(row_num != m.rowNum(), "AzPmat::copy_scol(m,pia)", "#rows mismatch"); 
  u._copy_scol2dcol(_dptr_u(), m._dptr(), row_num, pia_scol._dptr(), NULL, pia_scol.size()); 
}
void AzPmat::copy_scol(const AzPmat &m, int row_num, const AzPintArr &pia_scol) {
  int sz = row_num * pia_scol.size(); 
  AzX::throw_if(m.size() != sz || size() != sz, "AzPmat::copy_scol(m,#row,pia_scol)", "size mismatch"); 
  u._copy_scol2dcol(_dptr_u(), m._dptr(), row_num, pia_scol._dptr(), NULL, pia_scol.size()); 
}
...
/*------------------------------------------------*/
void AzPmat::add_col(const AzPmat *m2, double coeff) {
  const char *eyec = "AzPmat::add_col"; 
  AzX::throw_if_null(m2, eyec, "m2"); 
  AzX::throw_if((row_num != m2->row_num || m2->colNum() != 1), eyec, "wrong shape."); 
  u._add_col(_dptr_u(), row_num, col_num, m2->_dptr(), (AzFloat)coeff); 
}

/*------------------------------------------------*/
void AzPmat::add_row(const AzPmat *m2, double coeff) {
  const char *eyec = "AzPmat::add_row"; 
  AzX::throw_if_null(m2, eyec, "m2"); 
  AzX::throw_if((col_num != m2->col_num || m2->rowNum() != 1), eyec, "wrong shape."); 
  u._add_row(_dptr_u(), row_num, col_num, m2->_dptr(), (AzFloat)coeff); 
}
...
/*------------------------------------------------*/
void AzPmat::adam_delta(const AzPmat *m_g1, const AzPmat *m_g2, double b1t, double b2t, double eps, bool do_var) {
  ...
  set(m_g1); 
  u._adam_delta(size(), _dptr_u(), m_g2->_dptr(), (AzFloat)b1t, (AzFloat)b2t, (AzFloat)eps, do_var); 
}
...
void AzPmat::mark_ge_colth(const AzPmat *m, AzFloat coeff) { 
  const char *eyec = "AzPmat::mark_ge_colth(pmat)"; 
  AzX::throw_if_null(m, eyec, "colth"); AzX::throw_if((m->rowNum()!=1||m->colNum()!=col_num), eyec, "dimension mismatch"); 
  u._mark_ge_colth(_dptr_u(), row_num, col_num, m->_dptr(), coeff); 
}
void AzPmat::set_onehot(int r_num, const AzIntArr &ia_rows, AzFloat val) {
  const char *eyec = "AzPmat::set_onehot"; 
  AzX::throw_if(r_num<=0, eyec, "#row must be positive"); 
  reform(r_num, ia_rows.size()); 
  if (ia_rows.max() < 0) return; /* all zero */
  AzX::throw_if(ia_rows.max() >= r_num, eyec, "row index is out of range"); 
  AzPintArr pia_rows(&ia_rows); 
  u._set_onehot(_dptr_u(), row_num, col_num, pia_rows._dptr(), val); 
}
...
void AzPmat::sum_per_col(const AzPmat &m_inp) /* this <- colSum(inp) */ {
  AzPmat m_one((int)1, m_inp.row_num, (double)1);
  prod(&m_one, &m_inp, false, false); 
}

/*------------------------------------------------*/
/* sum over columns for each row */
void AzPmat::sum_per_row(const AzPmat &m_inp) /* this <- rowSum(inp) */ {
  AzPmat m_one(m_inp.col_num, (int)1, (double)1); 
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
void AzPmat::set_subcol_divcol(const AzPmat &mi, const AzPmat &vA, const AzPmat &vB) {
  reform_noinit(mi.row_num, mi.col_num); 
  if (mi.size() <= 0) return; 
  AzX::throw_if(vA.row_num != row_num || vB.row_num != mi.row_num || 
                vA.col_num != 1 || vB.col_num != 1, "AzPmat::set_subcol_divcol", "wrong shpae");   
  u._set_subcol_divcol(_dptr_u(), row_num, col_num, mi._dptr(), vA._dptr(), vB._dptr());                            
}  
void AzPmat::set_mulcol_addcol(const AzPmat &mi, const AzPmat &vA, const AzPmat &vB) {
  reform_noinit(mi.row_num, mi.col_num); 
  if (mi.size() <= 0) return; 
  AzX::throw_if(vA.row_num != row_num || vB.row_num != row_num || 
                vA.col_num != 1 || vB.col_num != 1, "AzPmat::set_mulcol_addcol", "wrong shpae");   
  u._set_mulcol_addcol(_dptr_u(), row_num, col_num, mi._dptr(), vA._dptr(), vB._dptr());                                
} 
void AzPmat::set_mulcol(const AzPmat &mi, const AzPmat &vA) {
  reform_noinit(mi.row_num, mi.col_num); 
  if (mi.size() <= 0) return; 
  AzX::throw_if(vA.row_num != row_num || vA.col_num != 1, "AzPmat::set_mulcol", "wrong shpae");   
  u._set_mulcol(_dptr_u(), row_num, col_num, mi._dptr(), vA._dptr());                                
}  
void AzPmat::set_mul(const AzPmat &mi0, const AzPmat &mi1) {
  AzX::throw_if(mi0.row_num != mi1.row_num || mi0.col_num != mi1.col_num, 
                "AzPmat::set_mul", "wrong shpae");    
  reform_noinit(mi0.row_num, mi0.col_num); 
  if (mi0.size() <= 0) return; 
  u._set_mul(_dptr_u(), row_num*col_num, mi0._dptr(), mi1._dptr());                                
}  
void AzPmat::set_subcol_sub_divcol(const AzPmat &mi0, const AzPmat &vA, const AzPmat &mi1, const AzPmat &vB) {
  AzX::throw_if(mi0.row_num != mi1.row_num || mi0.col_num != mi1.col_num ||
                vA.row_num != mi0.row_num || vB.row_num != mi0.row_num || vA.col_num != 1 || vB.col_num != 1, 
                "AzPmat::set_sub_subcol_divcol", "wrong shpae");
  reform_noinit(mi0.row_num, mi0.col_num); 
  if (mi0.size() <= 0) return;  
  u._set_subcol_sub_divcol(_dptr_u(), row_num, col_num, mi0._dptr(), vA._dptr(), mi1._dptr(), vB._dptr());
}  
void AzPmat::set_square(const AzPmat &mi) {
  reform_noinit(mi.row_num, mi.col_num); 
  if (mi.size() <= 0) return; 
  u._set_square(_dptr_u(), mi.size(), mi._dptr()); 
}
void AzPmat::to_binlogi_deriv(double y) {
  u._to_binlogi_deriv(_dptr_u(), size(), (AzFloat)y); 
}
void AzPmat::logloss_deriv(const AzPmat &mp, const AzPmat &my, AzPmat *m_loss) {
  const char *eyec = "AzPmat::logloss"; 
  mp.shape_chk_tmpl(&my, eyec, "p vs. y"); 
  reform_noinit(mp.rowNum(), mp.colNum()); 
  AzFloat *loss = NULL; 
  if (m_loss != NULL) {
    m_loss->reform(mp.rowNum(), mp.colNum()); 
    loss = m_loss->_dptr_u();     
  }  
  u._logloss(mp._dptr(), mp.size(), my._dptr(), _dptr_u(), loss); 
}

/*---------------------------------------------------------------------------*/
double AzPmat::trace() const {
  AzX::throw_if(row_num != col_num, "AzPmat::trace", "not square"); 
  _AzParr<AzFloat> diag; diag.free_alloc(row_num); 
  u._get_diagonal(_dptr(), row_num, diag._dptr_u()); 
  AzBaseArr<AzFloat> h_diag(row_num); 
  diag.copy_to_host(&h_diag); 
  double tr = 0; 
  for (int ix = 0; ix < row_num; ++ix) tr += h_diag[ix]; 
  return tr; 
}

/*---------------------------------------------------------------------------*/
void AzPmat::set_diag(const AzPmat &m_diag) {
  reform(m_diag.size(), m_diag.size()); 
  if (size() <= 0) return; 
  u._set_diagonal(m_diag._dptr(), row_num, _dptr_u());   
}
void AzPmat::get_diag(AzPmat &m_diag) const {
  AzX::throw_if(row_num != col_num, "AzPmat::get_diag", "not square"); 
  m_diag.reform(row_num, 1); 
  if (row_num <= 0) return; 
  u._get_diagonal(_dptr(), row_num, m_diag._dptr_u());   
}
void AzPmat::add_diag(const AzPmat &m_diag) {
  AzX::throw_if(row_num != m_diag.size() || col_num != m_diag.size(), "AzPmat::add_diag", "dim mistmatch"); 
  if (size() <= 0) return; 
  u._add_diagonal(m_diag._dptr(), row_num, _dptr_u());   
}

/*---------------------------------------------------------------------------*/
double AzPmat::sym_dist() const {
  if (row_num == 0 && col_num == 0) return 0; 
  if (row_num != col_num) return -1; 

  AzPmat m; m.transpose_from(this); 
  m.sub(this); 
  return m.absmax(); 
}
...
/*---------------------------------------------------------------------------*/  
/*---------------------------------------------------------------------------*/
void AzPmat::sym_eigenvalues(AzPmat &m_lam) const {
  const char *eyec = "AzPmat::sym_eigenvalues"; 
  AzPmat m(this); 
  m_lam.reform(row_num, 1); 
  AzPintArr pia_devinfo; pia_devinfo.free_alloc(1); 
#ifdef __AZ_EIGEN__
  bool do_valueonly = true; 
  u._sym1_eigen(m._dptr_u(), row_num, m_lam._dptr_u(), do_valueonly, pia_devinfo._dptr_u()); 
#else
  AzX::no_support(true, eyec, "_sym1_eigen"); 
#endif  
  AzIntArr ia_devinfo; 
  pia_devinfo.get(&ia_devinfo); 
  AzX::throw_if(ia_devinfo.size() < 1 || ia_devinfo[0] != 0, eyec, "sym1_eigen set error in devinfo"); 
}

/*---------------------------------------------------------------------------*/
/* static */
void AzPmat::sym_eigenvalues(AzPmat &m_A, AzPmat &m_B, /* destroyed */
                             AzPmat &m_lam, 
                             bool do_AxBx, 
                             AzBytArr *s_errmsg) {
  const char *eyec = "AzPmat::sym_eigenvalues";                                  
  int row_num = m_A.rowNum(); 
  AzX::throw_if(m_A.colNum() != row_num || m_B.rowNum() != row_num || m_B.colNum() != row_num, 
                eyec, "dimension mismatch");   
  m_lam.reform(row_num, 1); 
  AzPintArr pia_devinfo; pia_devinfo.free_alloc(1); 
#ifdef __AZ_EIGEN__
  bool do_valueonly = true; 
  m_A.u._sym2_eigen(m_A._dptr_u(), m_B._dptr_u(), row_num, do_AxBx, m_lam._dptr_u(), 
                    do_valueonly, pia_devinfo._dptr_u()); 
#else
  AzX::no_support(true, eyec, "_sym2_eigen"); 
#endif  
  AzIntArr ia_devinfo; 
  pia_devinfo.get(&ia_devinfo); 

  if (s_errmsg != NULL) s_errmsg->reset(); 
  if (ia_devinfo[0] != 0) {
    AzBytArr s("devinfo="); s << ia_devinfo[0];     
    if (s_errmsg != NULL) {
      s_errmsg->reset(eyec, ", sym2_eigen set error in devinfo: ", s.c_str()); 
    }
    else {
      AzX::throw_if(true, eyec, "sym2_eigen set error in devinfo: ", s.c_str()); 
    }
  }
}
