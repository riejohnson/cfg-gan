...
void AzpReNet::sup_training_loop(AzpData_ *trn,  /* not const b/c of first|next_batch */
  ...
  for (int ite = init_ite; ite < ite_num; ++ite) {  /* epochs */
    ...
    change_stepsize_if_required(&sssch, ite, &out);       
    trn->next_batch(); 
  }
  if (ite_num == 0) save(0); 
}
...
/*------------------------------------------------------------*/ 
double AzpReNet::change_stepsize_if_required(AzStepszSch *sssch, int ite, const AzOut *_out) {
  double coeff = sssch->get_stepsize_coeff(ite+1); 
  if (coeff > 0) {
    AzBytArr s("Setting step-sizes to s0 times "); s.cn(coeff); 
    if (_out != NULL) AzTimeLog::print(s, *_out); 
    const AzOut *myout = (dmp_out.isNull()) ? NULL : &out; 
    lays_multiply_to_stepsize(coeff, myout); 
  }
  return coeff; 
...
/*------------------------------------------------------------*/ 
void AzpReNet::show_layer_stat(const char *str, bool do_force) const {
  if (!do_force && do_less_verbose) return; 
  AzBytArr s(str); 
  lays_show_stat(s); 
...
void AzpReNet::up_nomc(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out,
  ...
  for (int lx = 1; lx < lays->size(); ++lx) {   
    ...
    if (ii != NULL) ii->set_if_needed(lx, mv_out);   
    mv_out.set(&mv); 
    _tLs(l_Upward, lx); 
  }
  _tTs(t_Upward); 
...
/*------------------------------------------------------------*/ 
int AzpReNet::last_lno() const {
  if (do_topthru && lays->size() < 2 || !do_topthru && lays->size() < 1) return -1; 
  if (mc.is_multi_conn()) {
    const AzIntArr &ia_order = mc.order();
    int top = ia_order[ia_order.size()-1]; 
    if (do_topthru) return mc.below(top); 
    else            return top; 
  }
  else {
    if (do_topthru) return lays->size()-2; 
    else            return lays->size()-1; 
  } 
}
...
void AzpReNet::up_mc(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out, 
  ...
  for (int ix = 1; ix < ia_order.size(); ++ix) {
    ...
    if (lx < lsz) {
      ...
      if (ii != NULL && below >= 0) ii->set_if_needed(lx, *amv[below]);     
    }
...
//    AzXi::throw_if_nonpositive(hid_num, eyec, kw_hid_num);         
  ...
  _resetParam(azp, is_alone);  /* read parameters used both train_test and test */
...
void AzpReNet::printParam(const AzOut &out, bool is_alone) const {
  ...
  _printParam(o, is_alone); /* show parameters used both for train_test and test */
...
void AzpReNet::_resetParam(AzParam &azp, bool is_alone) {
...
/*------------------------------------------------------------*/ 
/* parameters used for both train_test and test */
void AzpReNet::_printParam(AzPrint &o, bool is_alone) const {
  if (!is_alone) return; 
...
/*------------------------------------------------------------*/ 
/*------------------------------------------------------------*/ 
int AzpReNet::init_test(AzParam &azp, const AzpData_tmpl_ *tst, bool is_alone) {
  ...
  /*---  read parameters  ---*/
  resetParam_test(azp, is_alone); 
  printParam_test(out, is_alone); 
...
/*------------------------------------------------------------*/ 
void AzpReNet::resetParam_test(AzParam &azp, bool is_alone) {
  _resetParam(azp, is_alone); 
}  

/*------------------------------------------------------------*/ 
void AzpReNet::printParam_test(const AzOut &out, bool is_alone) const {
  AzPrint o(out); 
  _printParam(o, is_alone); 
  mc.printParam(out);  /* for multi-connection */ 
  o.ppEnd(); 
}
...
void AzpReNet::check_word_mapping(bool is_warmstart, const AzpData_tmpl_ *trn, 
  ...
  AzTimeLog::print("Checking the mapping ... ", out); 
  ...
  AzTimeLog::print("Checking the mapping of layers ... ", out); 
