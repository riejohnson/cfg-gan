...
public:
  ...
  virtual void resetParam_data(AzParam &azp) {
    const char *eyec = "AzpData_imgbin::resetParam_data"; 
...
protected:   
  virtual void _reset_data(int batch_no) {  
    ...
    /*---  Y  ---*/
    if (dummy_ydim > 0) {
      ms_y.reform(dummy_ydim, bin_x.dataNum()); 
    }
    else {
  ...
  virtual void _gen_data(const int *, int, AzPmatSpa *, int) const { no_support("AzpData_imgbin", "_gen_data(sparse)"); }
  virtual void _gen_data(const int *, int, AzPmatSpaVar *, int) const { no_support("AzpData_imgbin", "_gen_data(sparse and variable-sized)"); }
}; 
#endif 
