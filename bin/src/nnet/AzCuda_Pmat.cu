...
#include "AzPrint.hpp"
  ...
  __global__ void azc_add_col(AzFloat *dst, int r_num, int c_num, const AzFloat *src, AzFloat coeff) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) dst[ex] += coeff*src[ex%r_num]; 
  }
  void azccall_add_col(AzFloat *dst, int r_num, int c_num, const AzFloat *src, AzFloat coeff) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_add_col"); 
    azc_kernel(azc_add_col,bb,tt)(dst, r_num, c_num, src, coeff); chk_err("_add_col",bb,tt); 
  }
  
  /*---  add a row vector to every row  ---*/
  __global__ void azc_add_row(AzFloat *dst, int r_num, int c_num, const AzFloat *src, AzFloat coeff) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) dst[ex] += coeff*src[ex/r_num]; 
  }
  void azccall_add_row(AzFloat *dst, int r_num, int c_num, const AzFloat *src, AzFloat coeff) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_add_row"); 
    azc_kernel(azc_add_row,bb,tt)(dst, r_num, c_num, src, coeff); chk_err("_add_row",bb,tt); 
  }  
  ...
  __global__ void azc_adam_delta(int num, AzFloat *g1, const AzFloat *g2, 
                                 double coeff, double eps) {    
    for (int i = azc_thno; i < num; i +=azc_thnum) g1[i] *= (coeff/(sqrt(g2[i])+eps));     
  } 
  void azccall_adam_delta(int num, AzFloat *g1, const AzFloat *g2, 
                           AzFloat b1t, AzFloat b2t, AzFloat eps, bool do_variant) {
    int bb, tt; azc_config(num, bb, tt, "_adam_delta"); 
    double coeff = sqrt(1-b2t)/(1-b1t), epshat = eps*sqrt(1-b2t); 
    /* empirically, whether do_varaint=false or true doesn't really matter ... */
    if (do_variant) azc_kernel(azc_adam_delta,bb,tt)(num, g1, g2, coeff, eps); /* tensorflow */
    else            azc_kernel(azc_adam_delta,bb,tt)(num, g1, g2, coeff, epshat); /* as in the paper */
  ...
  /*---  mark_ge_colth: (x[,col]>=v[col])?coeff:0  ---*/
  __global__ void azc_mark_ge_colth(AzFloat *dst, int r_num, int c_num, const AzFloat *col_th, AzFloat coeff) {
    int num = r_num*c_num;      
    for (int ix = azc_thno; ix < num; ix += azc_thnum) {
      int col = ix/r_num; 
      dst[ix] = (dst[ix] >= col_th[col]) ? coeff : 0; 
  ...
  void azccall_mark_ge_colth(AzFloat *dst, int r_num, int c_num, const AzFloat *col_th, AzFloat coeff) {
    if (r_num*c_num <= 0) return; 
    int bb,tt; azc_config(r_num*c_num, bb,tt, "_mark_ge_colth"); 
    azc_kernel(azc_mark_ge_colth,bb,tt)(dst, r_num, c_num, col_th, coeff); chk_err("_mark_ge_colth",bb,tt); 
  }
  
  /*---  set_onehot: x[rows[col],col]:=val  ---*/
  __global__ void azc_set_onehot(AzFloat *dst, int r_num, int c_num, const int *rows, AzFloat val) {    
    for (int col = azc_thno; col < c_num; col += azc_thnum) {
      int row = rows[col]; 
      if (row >= 0) _entry(row, col, dst, r_num) = val; 
    }
  }   
  void azccall_set_onehot(AzFloat *dst, int r_num, int c_num, const int *rows, AzFloat val) {
    if (r_num*c_num <= 0) return; 
    int bb,tt; azc_config(c_num, bb,tt, "_set_onehot"); 
    azc_kernel(azc_set_onehot,bb,tt)(dst, r_num, c_num, rows, val); chk_err("_set_onehot",bb,tt); 
  }  
  ...
  __global__ void azc_inverse(AzFloat *dst, int num) {  
    for (int ix = azc_thno; ix < num; ix += azc_thnum) {
      if (dst[ix] != 0) dst[ix] = (AzFloat)1/dst[ix]; 
    }
  }
  ...
  /*----------------------------------------------------------------------------------------------------*/
  /*---  vecB must not have zero  ---*/
  __global__ void azc_set_subcol_divcol(AzFloat *dst, int r_num, int c_num, 
                                        const AzFloat *src, const AzFloat *vecA, const AzFloat *vecB) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) {
      int row = ex%r_num; 
      dst[ex] = (src[ex]-vecA[row])/vecB[row]; 
    }
  }
  void azccall_set_subcol_divcol(AzFloat *dst, int r_num, int c_num, 
                                 const AzFloat *src, const AzFloat *vecA, const AzFloat *vecB) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_subcol_divcol"); 
    azc_kernel(azc_set_subcol_divcol,bb,tt)(dst, r_num, c_num, src, vecA, vecB); chk_err("_set_subcol_divcol",bb,tt); 
  }   

  __global__ void azc_set_mulcol_addcol(AzFloat *dst, int r_num, int c_num, 
                                        const AzFloat *src, const AzFloat *vecA, const AzFloat *vecB) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) {
      int row = ex%r_num; 
  ...
  void azccall_set_mulcol_addcol(AzFloat *dst, int r_num, int c_num, 
                                 const AzFloat *src, const AzFloat *vecA, const AzFloat *vecB) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_mulcol_addcol"); 
    azc_kernel(azc_set_mulcol_addcol,bb,tt)(dst, r_num, c_num, src, vecA, vecB); chk_err("_set_mulcol_addcol",bb,tt); 
  } 

  __global__ void azc_set_mulcol(AzFloat *dst, int r_num, int c_num, 
                                 const AzFloat *src, const AzFloat *vecA) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) {
      int row = ex%r_num; 
      dst[ex] = src[ex]*vecA[row]; 
    }
  }
  void azccall_set_mulcol(AzFloat *dst, int r_num, int c_num, const AzFloat *src, const AzFloat *vecA) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_mulcol"); 
    azc_kernel(azc_set_mulcol,bb,tt)(dst, r_num, c_num, src, vecA); chk_err("_set_mulcol",bb,tt); 
  }   
  
  __global__ void azc_set_mul(AzFloat *dst, int num, const AzFloat *src0, const AzFloat *src1) {  
    for (int ex = azc_thno; ex < num; ex += azc_thnum) dst[ex] = src0[ex]*src1[ex]; 
  }
  void azccall_set_mul(AzFloat *dst, int num, const AzFloat *src0, const AzFloat *src1) {
    if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_mul"); 
    azc_kernel(azc_set_mul,bb,tt)(dst, num, src0, src1); chk_err("_set_mul",bb,tt); 
  }  
  
  /*---  vecB must not have zero ---*/
  __global__ void azc_set_subcol_sub_divcol(AzFloat *dst, int r_num, int c_num, 
                  const AzFloat *mat0, const AzFloat *vecA, const AzFloat *mat1, const AzFloat *vecB) {  
    int num = r_num*c_num; 
    for (int ex = azc_thno; ex < num; ex += azc_thnum) {
      int row = ex%r_num; 
      dst[ex] = (mat0[ex]-vecA[row]-mat1[ex])/vecB[row]; 
    }
  }
  /*---  vecB must not have zero ---*/  
  void azccall_set_subcol_sub_divcol(AzFloat *dst, int r_num, int c_num, const AzFloat *mat0, 
                                 const AzFloat *vecA, const AzFloat *mat1, const AzFloat *vecB) {
    int num = r_num*c_num; if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_subcol_sub_divcol"); 
    azc_kernel(azc_set_subcol_sub_divcol,bb,tt)(dst, r_num, c_num, mat0, vecA, mat1, vecB);
    chk_err("_set_subcol_sub_divcol",bb,tt); 
  }  
  __global__ void azc_set_square(AzFloat *dst, int num, const AzFloat *src) {
    for (int ex = azc_thno; ex < num; ex += azc_thnum) dst[ex] = src[ex]*src[ex];       
  }
  void azccall_set_square(AzFloat *dst, int num, const AzFloat *src) {
    if (num <= 0) return; int bb,tt; azc_config(num, bb,tt, "_set_square"); 
    azc_kernel(azc_set_square,bb,tt)(dst, num, src); 
    chk_err("_set_square",bb,tt);   
  }   
  /*------------------------------------------------------------*/
  __global__ void azc_binlogi_consty_deriv(AzFloat *p, int num, AzFloat y) {
    for (int ex = azc_thno; ex < num; ex += azc_thnum) p[ex] = -y/(exp(y*p[ex])+1);
  }
  void azccall_to_binlogi_deriv(AzFloat *p, int num, AzFloat y) {
    if (num <= 0) return; int bb, tt; azc_config(num, bb, tt, "_binlogi_consty_deriv"); 
    azc_kernel(azc_binlogi_consty_deriv,bb,tt)(p, num, y); 
    chk_err("_binlogi_consty_deriv", bb, tt);         
  }
  /* assume 0<p[i]<1 and 0<y[i]<1 */
  /*------------------------------------------------------------*/
  __global__ void azc_logloss(const AzFloat *p, int num, const AzFloat *y, 
                      AzFloat *deriv, AzFloat *loss) {
    for (int ex = azc_thno; ex < num; ex += azc_thnum) {
      if (deriv != NULL) deriv[ex] = (p[ex]-y[ex])/p[ex]/(1-p[ex]); 
      if (loss != NULL)  loss[ex] = -y[ex]*log(p[ex]) - (1-y[ex])*log(1-p[ex]); 
    }
  }
  void azccall_logloss(const AzFloat *p, int num, const AzFloat *y, 
                      AzFloat *deriv, AzFloat *loss) {
    if (num <= 0) return; int bb, tt; azc_config(num, bb, tt, "_logloss"); 
    azc_kernel(azc_logloss,bb,tt)(p, num, y, deriv, loss); chk_err("_logloss", bb, tt);         
  }  

  /*---  get diagonal components  ---*/
  __global__ void azc_get_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    for (int ex = azc_thno; ex < rc_num; ex += azc_thnum) dst[ex] = _entry(ex, ex, src, rc_num); 
  }   
  void azccall_get_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    if (rc_num <= 0) return; int bb, tt; azc_config(rc_num, bb, tt, "_get_diagonal");   
    azc_kernel(azc_get_diagonal,bb,tt)(src, rc_num, dst); chk_err("_get_diagonal",bb,tt); 
  } 

  /*---  set diagonal components  ---*/
  __global__ void azc_set_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    for (int ex = azc_thno; ex < rc_num; ex += azc_thnum) _entry(ex, ex, dst, rc_num) = src[ex]; 
  }   
  void azccall_set_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    if (rc_num <= 0) return; int bb, tt; azc_config(rc_num, bb, tt, "_set_diagonal");   
    azc_kernel(azc_set_diagonal,bb,tt)(src, rc_num, dst); chk_err("_set_diagonal",bb,tt); 
  }

  /*---  add diagonal components  ---*/
  __global__ void azc_add_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    for (int ex = azc_thno; ex < rc_num; ex += azc_thnum) _entry(ex, ex, dst, rc_num) += src[ex]; 
  }   
  void azccall_add_diagonal(const AzFloat *src, int rc_num, AzFloat *dst) {
    if (rc_num <= 0) return; int bb, tt; azc_config(rc_num, bb, tt, "_add_diagonal");   
    azc_kernel(azc_add_diagonal,bb,tt)(src, rc_num, dst); chk_err("_add_diagonal",bb,tt); 
  }  
