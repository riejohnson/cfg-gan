...
/*------------------------------------------------------------*/
void AzPmatApp::pooling_max_up2(const AzPmat &m1, int width1, 
                         AzPmat &m2, 
                         const AzPintArr2 &aIa_col1, /* only for getting width2 */
                         const AzPintArr &m_chosen) const {
  const char *eyec = "AzPmatApp::pooling_max_up2"; 
  ...
  a._pooling_max_up2(data_num, m1._dptr(), width1, m2._dptr_u(), width2, m1.row_num, 
                 m_chosen._dptr());  
}
...
/*------------------------------------------------------------*/
void AzPmatApp::pooling_max_var_up2(const AzPmat &m1, AzPmat &m2, 
                         const AzIntArr &ia_pos1, const AzIntArr &ia_pos2,                          
                         const AzPintArr &pia_chosen) const {
  const char *eyec = "AzPmatApp::pooling_max_var_up2"; 
  int maxsz2, cnum2 = prep_for_var(eyec, m1, m2, ia_pos1, ia_pos2, maxsz2); 
  AzPintArr pia_c2dx; gen_col2dx(ia_pos2, pia_c2dx, eyec); 
  ...
  a._pooling_max_var_up2(p, cnum2, pia_chosen._dptr()); 
}
