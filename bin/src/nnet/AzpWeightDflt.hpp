...
public:    
  ...
  virtual bool is_ready() const { return  (lms_const.size() > 0); }
protected:   
...
public:  
  ...
  virtual void upward(bool is_test, const AzPmat *m_x, AzPmat *m_out, const AzpLm_i *lmi=NULL) {
    if (do_thru) {
      m_out->set(m_x); 
      return;      
    }
    m_out->reform_noinit(classNum(), m_x->colNum()); 
    lm()->apply(m_x, m_out, lmi); 
  }
  ...
  virtual void downward(const AzPmat *m_lossd, AzPmat *m_d, const AzpLm_i *lmi=NULL) const {
    if (do_thru) {
      m_d->set(m_lossd); 
      return;       
    }
    m_d->reform_noinit(get_dim(), m_lossd->colNum()); 
    lm_const()->unapply(m_d, m_lossd, lmi); 
  }  
  virtual void updateDelta(int d_num, const AzPmat *m_x, const AzPmat *m_lossd, const AzpLm_i *lmi=NULL) { 
    if (do_thru) return; 
    if (lmods_sgd.size() == 1) lmods_sgd(0)->updateDelta(d_num, p, m_x, m_lossd, lmi, &ps); 
    else                       lm()->updateDelta(d_num, p, m_x, m_lossd, lmi);           
  }
  virtual void updateDelta(int d_num, const AzPmatSpa *m_x, const AzPmat *m_lossd, const AzpLm_i *lmi=NULL) { 
    if (do_thru) return; 
