...
public:
  ...
  virtual void upward2(const AzPmat &m_inp, AzPmat &m_out) { _up2(m_inp, m_out); }   
...
protected:
  ...
  virtual void _up2(const AzPmat &m_inp, AzPmat &m_out) const {
    m_out.reform(m_inp.rowNum(), m_inp.colNum()/input_size()*output_size()); 
    if      (p.ptyp == AzpPoolingDflt_Avg) upward_avg(&m_inp, &m_out); 
    else if (p.ptyp == AzpPoolingDflt_Max) up2_max(m_inp, m_out); 
    else AzX::no_support(true, "AzpPoolingDflt::_up2", "up2 (use Avg or Max)");  
  }  
  ...
  inline void up2_max(const AzPmat &m_inp, AzPmat &m_out) const {
    app.pooling_max_up2(m_inp, input_size(), m_out, pia2_out2inp, pia_chosen); 
  }   
