...
#define cusolverDnXsyevd_bufferSize cusolverDnDsyevd_bufferSize
#define cusolverDnXsyevd            cusolverDnDsyevd
#define cusolverDnXsygvd_bufferSize cusolverDnDsygvd_bufferSize
#define cusolverDnXsygvd            cusolverDnDsygvd
#define cusolverDnXsyevj_bufferSize cusolverDnDsyevj_bufferSize
#define cusolverDnXsyevj            cusolverDnDsyevj
#else
/*---  single-precision  ---*/
...
#define cusolverDnXsyevd_bufferSize cusolverDnSsyevd_bufferSize
#define cusolverDnXsyevd            cusolverDnSsyevd
#define cusolverDnXsygvd_bufferSize cusolverDnSsygvd_bufferSize
#define cusolverDnXsygvd            cusolverDnSsygvd
#define cusolverDnXsyevj_bufferSize cusolverDnSsyevj_bufferSize
#define cusolverDnXsyevj            cusolverDnSsyevj
#endif
...
void _AzPmat::_dbl_sym_eigen(double *inout, /* input: A (symmetric), output: eigenvectors */
  ...
  cusolverStatus_t ret = cusolverDnCreate(&solver); 
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnCreate"); 
  
  cusolverEigMode_t jobz = (do_valueonly) ? CUSOLVER_EIG_MODE_NOVECTOR : CUSOLVER_EIG_MODE_VECTOR; 
  cublasFillMode_t uplo = CUBLAS_FILL_MODE_UPPER; 
  int lwork = 0; 
  ret = cusolverDnDsyevd_bufferSize(solver, jobz, uplo, 
                                    rc_num, inout, rc_num, lambdas, &lwork); 
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnDsyevd_bufferSize failed"); 
  _AzParr<double> work; work.free_alloc(lwork, eyec, "lwork"); /* work area */
    
  ret = cusolverDnDsyevd(solver, jobz, uplo, rc_num, inout, rc_num, lambdas, 
                         work._dptr_u(), lwork, devinfo);
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnDsyevd (eigen computation) failed");     

  ret = cusolverDnDestroy(solver); 
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnDestroy");   
}
...
void _AzPmat::_sym2_eigen(AzFloat *inoutA, /* input: A (symmetric), output: eigenvectors */
  ...
  if (inoutB == NULL) {
    ...
    AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsyevd_bufferSize failed"); 
    ...
    ret = cusolverDnXsyevd(solver, jobz, uplo, rc_num, inoutA, rc_num, lambdas, 
                           work._dptr_u(), lwork, devinfo);
    AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsyevd (eigen computation) failed");    
  }
  else {
    cusolverEigType_t  eigtyp = (do_AxBx) ? CUSOLVER_EIG_TYPE_1 : CUSOLVER_EIG_TYPE_2; 
    int lwork = 0; 
    ret = cusolverDnXsygvd_bufferSize(solver, eigtyp, jobz, uplo, 
                                      rc_num, inoutA, rc_num, inoutB, rc_num, lambdas, &lwork); 
    AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsygvd_bufferSize failed"); 
    _AzParr<AzFloat> work; work.free_alloc(lwork, eyec, "lwork"); /* work area */
    
    ret = cusolverDnXsygvd(solver, eigtyp, jobz, uplo, rc_num, inoutA, rc_num, inoutB, rc_num, lambdas, 
                           work._dptr_u(), lwork, devinfo);
    AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsygvd (eigen computation) failed");     
  }
...
void _AzPmat::_symj_eigen(AzFloat *inoutA, /* input: A (symmetric), output: eigenvectors */
  ...
  syevjInfo_t params; 
  ret = cusolverDnCreateSyevjInfo(&params);
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnCreateSyevjInfo"); 
  
  int lwork = 0; 
  ret = cusolverDnXsyevj_bufferSize(solver, jobz, uplo, 
                                    rc_num, inoutA, rc_num, lambdas, &lwork, params); 
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsyevj_bufferSize failed"); 
  _AzParr<AzFloat> work; work.free_alloc(lwork, eyec, "lwork"); /* work area */
    
  ret = cusolverDnXsyevj(solver, jobz, uplo, rc_num, inoutA, rc_num, lambdas, 
                         work._dptr_u(), lwork, devinfo, params);
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnSsyevj (eigen computation) failed");    

  ret = cusolverDnDestroySyevjInfo(params);
  AzCuda::throwIfsolverError(ret, eyec, "cusolverDnDestroySyevjInfo");   
