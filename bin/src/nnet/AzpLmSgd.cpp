                         ...
                         const AzpLm_i &lmi)
{
  ...
  double uws = lmi.uws(); 
  if (doing_partial()) {
    AzX::no_support(uws != 1, "AzpLmSgd::_updateDelta", "partial with weight scaling"); 
    ...
    AzPs::prod(&m_w_grad, m_x, m_deriv, false, true, uws); 
  }
  
  bool do_bias = (!p.do_no_intercept && lmi.uis() != 0); 
  if (do_bias) {
    ...
    else v_i_grad.prod(&m_one, m_deriv, false, true, lmi.uis()); 
  }
  if      (p.do_nodiv)        grad_num = 1; 
  else if (p.do_count_regions) grad_num = m_x->colNum(); 
  else                         grad_num = d_num; 
  if (p.do_diveach) { /* divide at each updateDelta */
    AzX::throw_if(grad_num == 0, "AzpLmSgd::_updateDelta", "grad_num==0?!"); 
    m_w_grad.divide(grad_num); 
    if (do_bias) v_i_grad.divide(grad_num); 
    grad_num = 1;     
  }
} 
template void AzpLmSgd::_updateDelta<AzPmat>(int, const AzpLmParam &, const AzPmat *, const AzPmat *, const AzpLm_i &); 
template void AzpLmSgd::_updateDelta<AzPmatSpa>(int, const AzpLmParam &, const AzPmatSpa *, const AzPmat *, const AzpLm_i &); 

/*--------------------------------------------------------*/
void AzpLmSgd::_updateDelta(int d_num, const AzpLmParam &p, const AzPmat *m_grad, const AzpLm_i &lmi) {
  if (p.dont_update()) return;  
  const char *eyec = "AzpLmSgd::_updateDelta(grad)"; 
  AzX::no_support(doing_partial(), eyec, "Partial update"); 
  double alpha = (p.do_diveach) ? (double)1/(double)d_num : 1;  
  if (grad_num <= 0) {
    m_w_grad.set(m_grad, lmi.apply_uws(alpha)); 
    if      (p.do_diveach) grad_num = 1; 
    else if (p.do_nodiv)   grad_num = 1; 
    else                   grad_num = d_num;     
  }
  else {
    m_w_grad.add(m_grad, lmi.apply_uws(alpha)); 
    if      (p.do_diveach) grad_num = 1; 
    else if (p.do_nodiv)   grad_num = 1; 
    else                   grad_num += d_num;     
  }
...
void AzpLmSgd::_updateDelta2(int d_num, 
                         const AzpLmParam &p, 
                         const M *m_x, /* deriv if this is for double backprop */
                         const AzPmat *m_deriv, /* data if this is for double backprop */
                         const AzpLm_i &lmi)
{
  if (p.dont_update()) return;  
  ...
  double alpha = (p.do_diveach) ? (double)1/(double)d_num : 1; 
  AzPs::add_prod(&m_w_grad, m_x, m_deriv, false, true, lmi.apply_uws(alpha)); 
  if (!p.do_no_intercept && lmi.uis() != 0) {
    gen_one(m_x->colNum(), &m_one); 
    v_i_grad.add_prod(&m_one, m_deriv, false, true, lmi.apply_uis(alpha)); 
  }
  if      (p.do_diveach)       grad_num = 1; 
  else if (p.do_nodiv)         grad_num = 1; 
  else if (p.do_count_regions) grad_num += m_x->colNum(); 
...
template void AzpLmSgd::_updateDelta2<AzPmat>(int, const AzpLmParam &, const AzPmat *, const AzPmat *, const AzpLm_i &); 
template void AzpLmSgd::_updateDelta2<AzPmatSpa>(int, const AzpLmParam &, const AzPmatSpa *, const AzPmat *, const AzpLm_i &); 
