...
protected: 
  double _uws, _uis; 
public:   
  AzpLm_i(double iuws, double iuis) : _uws(1), _uis(1) { _uws=iuws; _uis=iuis; }
  AzpLm_i(const AzpLm_i *inp)       : _uws(1), _uis(1) { reset(inp); }
  void reset(const AzpLm_i *inp) { 
    if (inp == NULL) return; 
    _uws = inp->_uws; _uis = inp->_uis; 
  }
  double apply_uws(double val) const { return (_uws == 1) ? val : val*_uws; }
  double apply_uis(double val) const { return (_uis == 1) ? val : val*_uis; }
  double uws() const { return _uws; }
  double uis() const { return _uis; }
  static double uws(const AzpLm_i *ptr) { return (ptr==NULL) ? 1 : ptr->_uws; }
  static double uis(const AzpLm_i *ptr) { return (ptr==NULL) ? 1 : ptr->_uis; }
};
...
public:
  ...
  /*------------------------------------------------------------*/      
  inline virtual void apply(const AzPmat *m_x, AzPmat *m_out, const AzpLm_i *lmi) /*one const one*/ {
    const AzPmat *mw = (doing_partial()) ? &m_w_part : &m_w; 
    const AzPmat *vi = (doing_partial()) ? &v_i_part : &v_i; 
    double uws = AzpLm_i::uws(lmi); 
    m_out->prod(mw, m_x, true, false, uws); 
    if (ws != 1) m_out->multiply(ws); 
    double uis = AzpLm_i::uis(lmi); 
    if (uis != 0) {
      gen_one(m_x, &m_one);     
      m_out->add_prod(vi, &m_one, true, false, uis);    
    }
  }
  ...
  /*------------------------------------------------------------*/   
  inline virtual void unapply(AzPmat *m_d, const AzPmat *m_lossd, const AzpLm_i *lmi) const {
    const AzPmat *mw = (doing_partial()) ? &m_w_part : &m_w; 
    double uws = AzpLm_i::uws(lmi); 
    m_d->prod(mw, m_lossd, false, false, uws);     
...
public:
  ...
  virtual void updateDelta(int, const AzpLmParam &, const AzPmatSpa *, const AzPmat *, const AzpLm_i *lmi, const void *pp=NULL) {
    untrainable("updateDelta(spa)"); 
  }
  virtual void updateDelta(int, const AzpLmParam &, const AzPmat *, const AzPmat *, const AzpLm_i *lmi, const void *pp=NULL) {
