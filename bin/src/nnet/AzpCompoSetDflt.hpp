...
protected: 
  ...
  AzpWeight_sgdlike<AzpLmAdam, AzpLmAdam_Param> weight_adam;
  
  AzpWeight_ *p_weight; 
  
public:   
  AzpCompoSetDflt() : p_weight(&weight_dflt) {}
  AzpCompoSetDflt(AzParam &azp, bool for_test, const AzOut &out) : p_weight(&weight_dflt) { reset(azp, for_test, out); }
  
  #define kw_do_adadelta "AdaDelta"
  #define kw_do_rmsp "Rmsp"  
  #define kw_do_adam "Adam" 
  #define kw_opt "optim="
  #define kw_sgd "SGD"
  void reset(AzParam &azp, bool for_test, const AzOut &out) {
    ...
    if (!for_test) {   
      ...
      else if (azp.isOn(kw_do_adam    )) s_opt.reset(kw_do_adam);     /* for compatibility */      
      AzPrint o(out); azp.vStr(o, kw_opt, s_opt); 
      AzStrPool sp(kw_do_adadelta, kw_do_rmsp, kw_sgd, kw_do_adam); 
      ...
      else if (s_opt.equals(kw_do_adam    )) p_weight = &weight_adam;       
    }
  }  
