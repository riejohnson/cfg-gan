...
public:
  ...
  virtual bool can_do_up2() const { return (p.ptyp == AzpPoolingDflt_Avg || p.ptyp == AzpPoolingDflt_Max); }
  ...
  virtual void upward2(const AzPmatVar &mv_inp, AzPmatVar &mv_out) {   
    if (p.ptyp == AzpPoolingDflt_Avg) upward(true, &mv_inp, &mv_out); 
    else if (_up2_var(mv_inp, mv_out)) {}
    else _up2(mv_inp, mv_out); 
  }
...
protected:
  ...
  virtual void _up2(const AzPmatVar &m_inp, AzPmatVar &m_out) { 
    AzX::no_support(p.ptyp != AzpPoolingDflt_Max, "AzpPoolingDflt_var::_up2", "up2 (use Avg or Max)"); 
    AzPintArr2 pia2_out2inp; AzIntArr ia_out_ind; 
    map(&m_inp, p.pl_num, p.pl_sz, p.pl_step, p.pl_pad, p.do_pl_force_shape, &ia_out_ind, NULL, &pia2_out2inp);                     
    const AzPmat *m_iitems = m_inp.data(); 
    AzPmat m_oitems(m_iitems->rowNum(), pia2_out2inp.size());   
    up2_max(*m_iitems, pia2_out2inp, m_oitems);  
    m_out.set(&m_oitems, &ia_out_ind); 
  }  
  ...
  inline void up2_max(const AzPmat &m_inp, const AzPintArr2 &pia2_out2inp, AzPmat &m_out) {
    app.pooling_max_up2(m_inp, m_inp.colNum(), m_out, pia2_out2inp, pia_chosen); 
  }  
  ...
  virtual bool _up2_var(const AzPmatVar &mv1, AzPmatVar &mv2) {   
    if (!_do_var()) return false; 
    AzX::no_support(p.ptyp != AzpPoolingDflt_Max, "AzpPoolingDflt_var::_up2_var", "up2 (use Max or Avg)"); 
    AzPmat m2; app.pooling_max_var_up2(*mv1.data(), m2, ia_ipos, ia_opos, pia_chosen); 
    mv2.set(&m2, &ia_out_ind); 
    return true;     
  }
