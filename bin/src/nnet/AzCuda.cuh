...
public:
  ...
  /*---------------------------------------------------------------*/
  static
  void throwIfsolverError(cusolverStatus_t ret, 
                                  const char *eyec, 
                                  const char *str1="") {
    if (ret == CUSOLVER_STATUS_SUCCESS) return;     
    const char *errmsg = ""; 
    if (ret == CUSOLVER_STATUS_NOT_INITIALIZED) errmsg = "CUSOLVER_STATUS_NOT_INITIALIZED"; 
    else if (ret == CUSOLVER_STATUS_ALLOC_FAILED) errmsg = "CUSOLVER_STATUS_ALLOC_FAILED"; 
    else if (ret == CUSOLVER_STATUS_INVALID_VALUE) errmsg = "CUSOLVER_STATUS_INVALID_VALUE"; 
    else if (ret == CUSOLVER_STATUS_ARCH_MISMATCH) errmsg = "CUSOLVER_STATUS_ARCH_MISMATCH"; 
    else if (ret == CUSOLVER_STATUS_EXECUTION_FAILED) errmsg = "CUSOLVER_STATUS_EXECUTION_FAILED"; 
    else if (ret == CUSOLVER_STATUS_INTERNAL_ERROR) errmsg = "CUSOLVER_STATUS_INTERNAL_ERROR"; 
    else if (ret == CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED) errmsg = "CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED";     
    else errmsg = "Unknown error"; 
    
    AzBytArr s("throwIfsolverError: "); s << str1 << " " << errmsg;  
    AzX::pthrow(AzCudaErr, eyec, s.c_str()); 
  }
