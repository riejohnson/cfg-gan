...
#define AzpData_Ext_txtbin ".txtbin"
...
public: 
  ...
  virtual void gen_data(const int *dxs, int d_num, AzPmatVar &mv_data, int dsno=0) const {
    _gen_data(dxs, d_num, &mv_data, dsno);  
  }
  virtual void gen_data(int dx_begin, int d_num, AzPmatVar &mv_data, int dsno=0) const {
    AzIntArr ia_dxs; ia_dxs.range(dx_begin, dx_begin+d_num); 
    _gen_data(ia_dxs.point(), ia_dxs.size(), &mv_data, dsno);  
  }
