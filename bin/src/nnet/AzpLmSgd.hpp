...
public:
  ...
  virtual void updateDelta(int d_num, const AzpLmParam &p, 
    ...
    AzX::throw_if(p.do_diveach && d_num == 0, "AzpLmSgd::updateDelta", "d_num==0"); 
    AzpLm_i lmi(_lmi); 
    if (grad_num <= 0) _updateDelta(d_num, p, m_x, m_deriv, lmi); 
    else               _updateDelta2(d_num, p, m_x, m_deriv, lmi); 
  }                           
  virtual void updateDelta(int d_num, const AzpLmParam &p, 
    ...
    AzX::throw_if(p.do_diveach && d_num == 0, "AzpLmSgd::updateDelta", "d_num==0");     
    AzpLm_i lmi(_lmi);    
    if (m_x == NULL)        _updateDelta(d_num, p, m_deriv, lmi); /* 12/04/2016 for bn */
    else if (grad_num <= 0) _updateDelta(d_num, p, m_x, m_deriv, lmi);
    else                    _updateDelta2(d_num, p, m_x, m_deriv, lmi); 
  } 
...
protected:  
  ...
  template <class M>
  void _updateDelta(int d_num, const AzpLmParam &p, const M *m_x, const AzPmat *m_deriv, const AzpLm_i &lmi);  
  template <class M>
  void _updateDelta2(int d_num, const AzpLmParam &p, const M *m_x, const AzPmat *m_deriv, const AzpLm_i &lmi); 
  void _updateDelta(int d_num, const AzpLmParam &p, const AzPmat *m_grad, const AzpLm_i &lmi); 
