...
public:
  ...
  AzPmat(int r_num, int c_num, double ini_val) : locked(false), row_num(0), col_num(0) { 
    reform_noinit(r_num, c_num); set(ini_val); 
  }  
  ...
  void copy_scol(const AzPmat &m, const AzPintArr &pia_scol); 
  void copy_scol(const AzPmat &m, int row_num, const AzPintArr &pia_scol); 
  ...
  void rbind(const AzDmat &md) { AzPmat m(&md); rbind(&m); }
  ...
  void add_col(const AzPmat *m2, double coeff=1); /* this[r,] += m2[r,0]*coeff */
  void add_row(const AzPmat *m2, double coeff=1); /* this[c,] += m2[0,c]*coeff */  
  ...
  void adam_delta(const AzPmat *m_g1, const AzPmat *m_g2, double b1t, double b2t, double eps, bool do_var);  /* this <- Adam update */
  ...
  void divide_row(const AzPmat *m2) { multiply_eachcol(m2, true); }
  ...
  void multi_row(const AzDmat &md) { AzPmat m2(&md); multi_row(&m2); }
  void multi_col(const AzDmat &md) { AzPmat m2(&md); multi_col(&m2); }  
  ...
  void mark_ge_colth(const AzPmat *m, AzFloat coeff); /* for every row: set (x[,c]>=m[0,c])?coeff:0 */   
  void set_onehot(int r_num, const AzIntArr &ia_rows, AzFloat val=1); 
  ...
  void zeroout_negative() { AzPmat m(this); m.mark_positive(); this->elm_multi(&m); }
  ...
  inline virtual void reform_noinit(int r_num, int c_num) { /* shape the matrix but don't initialize with zero */
    ...
    int sz = r_num*c_num;   
    double dblsz = (double)r_num*(double)c_num;     
    if (dblsz >= (double)2147483648) { /* overflow */
  ...
  /*---  legacy  ---*/
  void colSum(const AzPmat *m_inp) {
    AzX::throw_if_null(m_inp, "AzPmat::colSum", "input"); 
    sum_per_col(*m_inp); 
  }
  void rowSum(const AzPmat *m_inp) {
    AzX::throw_if_null(m_inp, "AzPmat::rowSum", "input"); 
    sum_per_row(*m_inp);     
  }
  /*---  newer  ---*/  
  void sum_per_col(const AzPmat &m_inp); /* sum the values for each column */
  void sum_per_row(const AzPmat &m_inp); /* sum the values for each row */
  void avg_per_col(const AzPmat &m_inp) { /* average the values for each column */
    sum_per_col(m_inp); if (m_inp.row_num != 0) divide((double)m_inp.row_num); 
  } 
  void avg_per_row(const AzPmat &m_inp) { /* average the values for each row */ 
    sum_per_row(m_inp); if (m_inp.col_num != 0) divide((double)m_inp.col_num); 
  } 
  /*---------------*/
  ...
  inline void prod(const AzPmat &m0, const AzPmat &m1, const AzPmat &m2, 
                   bool isTr0=false, bool isTr1=false, bool isTr2=false) {
    AzPmat m; m.prod(&m0, &m1, isTr0, isTr1); 
    prod(&m, &m2, false, isTr2);   
  }
  ...
  void set_subcol_divcol(const AzPmat &mi, const AzPmat &vA, const AzPmat &vB); 
  void set_mulcol_addcol(const AzPmat &mi, const AzPmat &vA, const AzPmat &vB); 
  void set_mulcol(const AzPmat &mi, const AzPmat &vA); 
  void set_mul(const AzPmat &mi0, const AzPmat &mi1); 
  void set_subcol_sub_divcol(const AzPmat &mi0, const AzPmat &vA, const AzPmat &mi1, const AzPmat &vB); 
  void set_square(const AzPmat &mi); 
  void to_binlogi_deriv(double y); 
  void logloss_deriv(const AzPmat &mp, const AzPmat &my, AzPmat *m_loss=NULL); 
  
  void set_identity(int dim) { AzPmat m_diag(dim, 1, (double)1); set_diag(m_diag); }
  void set_diag(const AzPmat &m_diag); 
  void set_diag(double val) { AzPmat m_diag(row_num, 1, val); set_diag(m_diag); }
  void get_diag(AzPmat &m_diag) const; 
  void add_diag(const AzPmat &m_diag);   
  void add_diag(double val) { AzPmat m_diag(row_num, 1, val); add_diag(m_diag); }
  double sym_dist() const; 
  double avg() { if (size() <= 0) return 0; return sum()/(double)size(); }
  void sym_eigenvalues(AzPmat &m_lam) const;  /* must be symmetric */
  static void sym_eigenvalues(AzPmat &m_A, /* just be symmetric.  destroyed */
                              AzPmat &m_B, /* just be symmetric positive definite.  destroyed */
                              AzPmat &m_lam, 
                              bool do_AxBx=false, /* false: ABV=Vlam, true: AV=BVlam */
                              AzBytArr *s_errmsg=NULL); 
protected: 
...
public:
  ...
  void cbind(const AzPmatVar *mv) {
    if (mv == NULL) return; 
    if (m.size() <= 0) { set(mv); return; }
    AzX::throw_if(mv->rowNum() != rowNum(), "AzPmatVar::cbind", "#row mismatch"); 
    ia_dcolind.concat(&mv->ia_dcolind, m.colNum()); 
...
protected: 
  ...
  int c_num; 
public:
  AzPmatVarData() : c_num(0) {}
  int dataNum() const { return _mv.dataNum(); }
  void reset() { _mv.reset(); c_num = 0; }
  void init_for_put_seq(const AzPmatVar &mv_tmpl, int data_num) {  
    c_num = mv_tmpl.size(0); 
    AzIntArr ia_dind; 
    for (int dx0 = 0; dx0 < data_num; ++dx0) { ia_dind.put(dx0*c_num); ia_dind.put((dx0+1)*c_num); }
    _mv.reform_noinit(mv_tmpl.rowNum(), &ia_dind); 
  }  
  void put_seq(int dx_begin, const AzPmatVar &mvi) {
    const char *eyec = "AzPmatVarData::put_seq"; 
    AzX::throw_if(dx_begin < 0 || dx_begin+mvi.dataNum() > _mv.dataNum(), 
                  eyec, "data# is out of range or too many data points"); 
    check_input(mvi, eyec); 
    _mv.data_u()->set(dx_begin*c_num, mvi.colNum(), mvi.data()); 
  }  
  void get(int dx, int d_num, AzPmatVar &mv) const {
    AzIntArr ia; ia.range(dx, dx+d_num); 
    get(ia, mv); 
  }
  void get(const AzIntArr &ia_dxs, AzPmatVar &mv) const {
    mv.set(&_mv, ia_dxs.point(), ia_dxs.size());   
  }
  void get(const int *dxs, int dxs_num, AzPmatVar &mv) const {
    AzX::throw_if_null(dxs, "AzPmatVarData::get"); 
    mv.set(&_mv, dxs, dxs_num);   
  }  
...
protected:
  void check_input(const AzPmatVar &mvi, const char *eyec) const { 
    ...
    for (int dx = 0; dx < mvi.dataNum(); ++dx) {
      AzX::throw_if(mvi.size(dx) != c_num, eyec, "Unexpected #cols per data.");  
    }
  }
};
