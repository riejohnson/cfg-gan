...
/*------------------------------------------------------------*/
/* interface for obtaining internal data */
class AzpReNet_ii_ {
public:
  virtual void set_if_needed(int lno, const AzPmatVar &mv) = 0; 
};
...
public:    
  virtual AzpReNet *safe_clone_nocopy(AzObjPtrArr<AzpReNet> &arr) const {
    arr.free_alloc(1);
    arr.set(0, clone_nocopy());  
    return arr(0);   
  }
  virtual void safe_clone_nocopy(AzObjPtrArr<AzpReNet> &arr, int num) const {
    arr.free_alloc(num);
    for (int ix = 0; ix < num; ++ix) arr.set(ix, clone_nocopy());  
  } 
  virtual void reset() {
  ...
  virtual int init(AzParam &azp, const AzpData_tmpl_ *trn, const AzpData_tmpl_ *tst=NULL, const AzpData_tmpl_ *tst2=NULL, 
                   bool is_alone=false); 
  virtual int init_noshow(AzParam &azp, const AzpData_tmpl_ *trn, const AzpData_tmpl_ *tst=NULL, const AzpData_tmpl_ *tst2=NULL, 
                   bool is_alone=false) {
    deactivate_out(); int ret=init(azp, trn, tst, tst2, is_alone); activate_out(); return ret; 
  }                   
  void deactivate_out() { out.deactivate(); }
  void activate_out() { out.activate(); }  
  virtual int init_test(AzParam &azp, const AzpData_tmpl_ *tst, bool is_alone=false); 
  virtual void up0(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out); /* first layer only */  
  virtual void up(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL) {
    if (mc.is_multi_conn()) up_mc(is_test, data, mv_out, ii); 
    else                    up_nomc(is_test, data, mv_out, ii); 
  }
  virtual void up(bool is_test, const AzPmatVar &mv_inp, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL) {
    AzDataArr<AzpDataVar_X> darr(1); AzpData_::set_data(mv_inp, *darr(0)); up(is_test, darr, mv_out, ii);  
  }
  virtual void up(bool is_test, const AzPmatVar &mv0, const AzPmatVar &mv1, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL) {
    AzDataArr<AzpDataVar_X> darr(2); AzpData_::set_data(mv0, *darr(0)); AzpData_::set_data(mv1, *darr(1)); up(is_test, darr, mv_out, ii);  
  }  
  virtual void up(bool is_test, const AzPmatSpaVar &msv_inp, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL) {
    AzDataArr<AzpDataVar_X> darr(1); AzpData_::set_data(msv_inp, *darr(0)); up(is_test, darr, mv_out, ii);  
  }  
  ...
  virtual void up2(const AzPmatVar &, AzPmatVar &, bool dont_update=false) { AzX::no_support(true,"AzpReNet::up2","up2"); }   
  virtual void inc_up(const AzPmat &m_inp, AzPmat &m_out) { AzX::no_support(true, "AzpReNet::in_up", "inc_up"); }
  virtual void inc_up_finish() { AzX::no_support(true, "AzpReNet::inc_up_finish", "inc_up_finish"); }
  ...
  virtual void multiply_to_stepsize(double coeff, const AzOut *_out=NULL) { 
    lays_multiply_to_stepsize(coeff, _out); 
  }
  
  virtual void save(int iteno); 
  virtual void show_layer_stat(const char *str=NULL, bool do_force=false) const;   
  virtual void reset_stepsize() { reset_stepsize(&sssch); }
  virtual double change_stepsize_if_required(int ite, const AzOut *_out=NULL) { 
    return change_stepsize_if_required(&sssch, ite, _out); 
  } 
  virtual double get_stepsize_coeff(int ite) { return sssch.get_stepsize_coeff(ite); }
  ...
  virtual const AzpReUpperLayer_ *layn(int lno) const { /* for gan */
    AzX::throw_if(lno < 0 || lno >= lays->size(), "AzpReNet::layn", "layer# is out of range");    
    return (*lays)[lno]; 
  }
  virtual int last_lno() const; /* the layer# of the last non-dummy layer */ 
protected:  
  ...
  virtual void up_mc(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL); 
  virtual void down_mc(const AzPmatVar &mv_ld, bool dont_update, bool dont_release_sv); 
  virtual void up_nomc(bool is_test, const AzDataArr<AzpDataVar_X> &data, AzPmatVar &mv_out, AzpReNet_ii_ *ii=NULL); 
  ...
  virtual double change_stepsize_if_required(AzStepszSch *sssch, int ite, const AzOut *_out);  
  ...
  virtual void resetParam_test(AzParam &azp, bool is_alone); 
  virtual void printParam_test(const AzOut &out, bool is_alone) const; 
  
  virtual void _resetParam(AzParam &azp, bool is_alone); 
  virtual void _printParam(AzPrint &o, bool is_alone) const; 
  /*-------------------------------------------------------*/
