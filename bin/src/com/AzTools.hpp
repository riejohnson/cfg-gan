/* * * * *
 *  AzTools.hpp 
 *  Copyright (C) 2011-2018 Rie Johnson
 *
...
public: 
  ...
  static void readText(const char *txt_fn, AzStrPool &sp); 
...
public:                                   
  ...
  static void shuffle2(AzIntArr &ia, AzRandGen *rg=NULL); 
  ...
  inline static void check_milestone(const AzOut &out, int &milestone, int index, int inc) {
    if (milestone > 0 && index >= milestone) {
      AzPrint::write(out, "."); out.flush(); 
      milestone += inc; 
    }
  }
  inline static void finish_milestone(const AzOut &out, int &milestone) {
    if (milestone > 0) AzPrint::writeln(out, ""); 
    milestone = -1; 
  }   
