...
/*--------------------------------------------------------*/
void AzTools::readText(const char *txt_fn, AzStrPool &sp) {
  AzIntArr ia_line_len; 
  AzFile::scan(txt_fn, 65536, &ia_line_len); 
  sp.reset(ia_line_len.size(), (ia_line_len.max()+ia_line_len.min())/2); 
  int buff_size = ia_line_len.max() + 256; 
  AzBytArr s_buff; AzByte *buff = s_buff.reset(buff_size, 0);  
  AzFile file(txt_fn); file.open("rb"); 
  for ( ; ; ) {
...
void AzTools::shuffle(int rand_seed, AzIntArr *iq, bool withReplacement) {
  ...
  if (!withReplacement) {
    AzX::no_support(iq->find(AzNone) >= 0, "AzTools::shuffle", "shuffle with AzNone"); 
  }
...
/*--------------------------------------------------------*/
/* without replacement */
void AzTools::shuffle2(AzIntArr &ia, AzRandGen *rg) {
  int num = ia.size(); 
  AzIntArr ia_idx; ia_idx.range(0, num); 
  for (int taken = 0; taken < num; ++taken) {
    int rand_no = (rg == NULL) ? rand_large() : rg->_rand_(); 
    rand_no = rand_no % (num-taken) + taken; 
