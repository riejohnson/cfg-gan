...
/*------------------------------------------------------------------*/
void AzStrPoolc::copy_to(AzStrPool &sp, AzByte dlm) const {
  const char *eyec = "AzStrPoolc::copy_to"; 
  sp.reset(num, data.size()/num); 
  const AzByte *ptr = data.point(), *data_end = ptr + data.size(); 
  for (int ix = 0; ix < num; ++ix) {
    const AzByte *ptr2 = (AzByte *)memchr(ptr, dlm, data_end-ptr); 
    AzX::throw_if(ptr2 == NULL, eyec, "Unexpected end of data"); 
    sp.put(ptr, Az64::ptr_diff(ptr2-ptr)); 
    ptr = ptr2 + 1; /* +1 for the delimiter */
  }
  AzX::throw_if(ptr != data_end, eyec, "More data?!"); 
}  
