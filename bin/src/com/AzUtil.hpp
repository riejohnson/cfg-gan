...
public:
  ...
  template <class A>
  inline static void print(const char *msg, A number, const AzOut &out) {
    AzBytArr s; s << number; 
...
public:
  ...
  void tick(double &oclk, double &otim) {
    clock_t b_clk=clk; time_t b_tim=tim; update(); /* save and update */
    otim += (double)(tim-b_tim); oclk += (double)(clk-b_clk)/(double)CLOCKS_PER_SEC; 
  }
