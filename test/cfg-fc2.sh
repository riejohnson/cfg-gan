...
#####  (Fig 3 of [Johnson+Zhang,ICML18]) 
  ...
  source func.sh
  gpumem=${gpu}:11  # pre-allocate 11GB device memory
...
#  nm=twbg;  imgsz=64; cc=3; batches=7  # 64x64 3-channel,  7 data batches
  nm=mnist; imgsz=32; cc=1; batches=1  # 32x32 (0 padded from 28x28) 1-channel 
#  nm=svhn;  imgsz=32; cc=3; batches=1  # 32x32 3-channel
  ...
  if [ "$nm" = "brlr" ] || [ "$nm" = "twbg" ]; then
    ...
    test_clk=5000  # test after every 5k seconds 
